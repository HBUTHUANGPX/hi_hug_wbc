...
p1 = torch.tensor([0.0, 0.0, -0.2])
p2 = torch.tensor([0.0, 0.0, -0.2])
tra1 = translate(p1)
tra2 = translate(p2)
...
l_angle[l_angle > 0] = 0
# l_angle += y_bias
r1_l = -l_angle * scale + 0.4
r2_l = l_angle * scale* 2 - 0.8
...
# angle[angle < 0] = 0
# angle -= y_bias
r1_r = angle * scale + 0.4
r2_r = -angle * scale * 2 - 0.8
...
# plt.plot(t_np[0,:],sin_pos_np[0, :] * 0.035, label="Sin Pos", marker=".")
...
# plt.plot(t_np[0, :], end_pos_r_np[0, :] - np.min(end_pos_r_np[0, :]), label="End pos r", marker=".")
# plt.plot(t_np[0,:],np.abs(end_pos_l_np[0, :] - end_pos_r_np[0, :]), label="derta", marker=".")
# 添加图例
plt.legend()
print(np.max(end_pos_l_np[0, :])-np.min(end_pos_l_np[0, :]))
# 添加标题和标签
